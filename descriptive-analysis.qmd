...
```{r message = FALSE, warning = FALSE}
table(sciscore$id_in_abstract)
table(sciscore$id_in_methods)
table(sciscore$id_in_other_location)
```
...
table(trialidentifier$id_in_abstract)
table(trialidentifier$id_in_methods)
table(trialidentifier$id_in_other_location)
```
...
table(ctregistries$id_in_abstract)
table(ctregistries$id_in_methods)
table(ctregistries$id_in_other_location)
```
...
table(nct$id_in_abstract)
table(nct$id_in_methods)
table(nct$id_in_other_location)
```
...
### Negative-control check: results

Overall, out of the 50 papers reviewed, there where only three papers flagged for a possible registration number:

1.  Number ID found for the Institutional Animal Care and Use Committee (IACUC) - this could be a protocol ID.

2.  Number ID from www.researchregistry.com.

3.  Number ID for Prospero Registry.
