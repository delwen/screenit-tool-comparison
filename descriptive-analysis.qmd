...
barplot(count_true, names.arg = colnames(location_analysis1), 
        col = custom_palette(length(count_true)), 
        main = "Location of IDs", ylab = "Count",
...
sciscore2 <- data %>% filter(sciscore_hit)
sciscore2 %>% count(id_in_abstract, id_in_methods, id_in_other_location)
```
...
Additionally, we compared the total number of ctgov IDs present in the methods section, to the ctgovs IDs present in methods section were sciscore_hit was TRUE, to see if the tool missed any of these IDs in this particular section:

```{r message = FALSE, warning = FALSE}

#dataset with only methods section = TRUE
methods_ids <- subset(data, data$id_in_methods %in% c("TRUE"))

#table ctgov in methods
methods_ids %>% 
  count(id_type) %>%
  arrange(desc(n)) %>%
  kable()

#table sciscore - ctgov in methods
methods_ids %>% 
  count(id_type, sciscore_hit) %>%
  arrange(desc(n)) %>%
  kable()

```

Out of 63 ctgov IDs present in the methods section, `Sciscore` found only 52 ctgov IDs in the method section.
